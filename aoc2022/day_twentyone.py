    ...
    while not found_root:
        for monkey, value in monkeys.items():
            if not isinstance(value, int):
                if isinstance(monkeys[value[0]], int) and isinstance(
                    monkeys[value[2]], int
                ):
...
def part_two(monkeys):
    ...
    while not found_root:
        for monkey, value in new_monkeys.items():
            if not isinstance(value, int):
                if not isinstance(value[0], int) and isinstance(
                    new_monkeys[value[0]], int
                ):
                    new_monkeys[monkey] = (new_monkeys[value[0]], value[1], value[2])

                if not isinstance(value[2], int) and isinstance(
                    new_monkeys[value[2]], int
                ):
                    new_monkeys[monkey] = (
                        new_monkeys[monkey][0],
                        value[1],
                        new_monkeys[value[2]],
                    )
