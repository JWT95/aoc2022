    ...
    for i in range(-2000000, 2000000):
        ...
        for sensor, beacon in sensors.items():
            ...
            if distance_line <= distance:
                banned_points.add(((max(sensor[0]-(distance-distance_line), 0)), min(sensor[0]+(distance-distance_line), 2*LINE)))
        ...
        for i in range(len(ranges) - 1):
            ...
            if current_max < ranges[i+1][0] - 1:
                final_ranges = [(ranges[0][0], current_max), (ranges[i+1][0], ranges[-1][1])]
